...
async def node_commit(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="auto" if state.get("escalation_answers") else str(state.get("human_choice", "pending")),
...
async def node_auto_approve(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="auto",
        reason="Auto-approved due to high confidence",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        reviewer_id=os.environ.get("GITHUB_USER"),
        decision="escalate",
        reason="Received reviewer answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_synthesize(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="pending",
        reason="Refined analysis using reviewer answers",
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
