    ...
    with console.status("[dim]LLM reviewing the diff...[/dim]"):
        analysis = llm.invoke([
            {"role": "system", "content": (
                ...
                "If confidence < 60%, populate escalation_questions with 2-4 "
                "specific, context-rich questions (reference which file/section in the diff)."
            )},
...
def node_synthesize(state: ReviewState) -> dict:
    ...
    llm = get_llm().with_structured_output(PRAnalysis)
    with console.status("[dim]LLM refining review...[/dim]"):
        refined = llm.invoke([
            {"role": "system", "content": "Refine the review using the human answers."},
