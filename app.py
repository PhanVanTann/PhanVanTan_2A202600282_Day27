...
from exercises.exercise_4_audit import build_graph, handle_interrupt
import aiosqlite
...
with st.sidebar:
    ...
    async def get_recent_threads():
        try:
            async with aiosqlite.connect(db_path()) as conn:
                ...
                async with conn.execute("SELECT thread_id, pr_url, max(timestamp) as last_event FROM audit_events GROUP BY thread_id ORDER BY last_event DESC LIMIT 10") as cur:
    ...
    for t in threads:
        if st.button(f"{t['pr_url'].split('/')[-1]} ({t['thread_id'][:8]})"):
